...
static void handle_exception(SceDebugRegBlock *regs);

#define MAX_BUF 2048

void _GdbExceptionHandler(void);
static SceDebugRegBlock *_GdbExceptRegs;
...
static void _GdbTrapEntry(SceDebugRegBlock *regs)
{
...
static void step_generic(SceDebugRegBlock *regs, int skip)
{
	u32 opcode;
	u32 epc;
	u32 targetpc;
...
void build_trap_cmd(int sigval, SceDebugRegBlock *regs)
{
	char *ptr;
	/*
