...
typedef struct _SceDebugRegBlock
{
	u32 frame[6];
...
typedef void (*SceDebugErrorHandler)(SceDebugRegBlock *regs);
...
/** 
  ...
  * @return < 0 on error
  */
int pspDebugInstallErrorHandler(SceDebugErrorHandler handler);
...
/**
  ...
  *
  */
void pspDebugDumpException(SceDebugRegBlock *regs);

/** Type for Kprintf handler */
typedef int (*SceDebugKprintfHandler)(const char *format, u32 *args);
...
/** 
  ...
  * @return < 0 on error.
  */
int pspDebugInstallKprintfHandler(SceDebugKprintfHandler handler);

/** Structure to hold a single stack trace entry */
typedef struct _SceDebugStackTrace
{
...
/**
  ...
  * @param trace - Pointer to an array of SceDebugStackTrace structures.
...
typedef struct _SceDebugProfilerRegs
{
	volatile u32 enable;
...
/** Get the profiler register state
  *
  * @param regs - A pointer to a SceDebugProfilerRegs structure.
  */
void pspDebugProfilerGetRegs(SceDebugProfilerRegs *regs);

/** Print the profiler registers to screen */
void pspDebugProfilerPrint(void);

/** Type for the debug print handlers */
typedef int (*SceDebugPrintHandler)(const char *data, int len);

/** Type for the debug input handler */
typedef int (*SceDebugInputHandler)(char *data, int len);
