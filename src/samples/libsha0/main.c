 ...
 * Ultros - 2024
 * 
 * To use libsha0 you must first acquire the libsha0.prx from somewhere, i got mine from "talkman"
...
PSP_MODULE_INFO("libsha0test", 0, 1, 1);
...
int main(void){
	...
	//holders
	unsigned char a_hash[SHA0_HASH_LEN];
	char a_buffer[3];
	
	//compute all segments in one pass
	sceSha0Digest((const unsigned char*)string, strlen((const char*)string), a_hash);
	...
	//holders
	unsigned char b_hash[SHA1_HASH_LEN];
	char b_buffer[3];
	
	//declare context for sha0 data
	sceSha0Context pContext;
	
	//init sha0 context
	sceSha0BlockInit(&pContext);
	
	//loop through the block of data from start to end, 
	//in this case its a string so we can use strlen to find the end.
	for(i=0;i<(strlen(string));i++){
		//compute segment
		sceSha0BlockUpdate(&pContext, (const unsigned char*)string+i, sizeof(unsigned char));
	}
	
	//get the final digest
	sceSha0BlockResult(&pContext, b_hash);
