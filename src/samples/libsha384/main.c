 ...
 * Ultros - 2024
 * 
 * To use libsha384 you must first acquire the libsha384.prx from somewhere, i got mine from "talkman"
...
PSP_MODULE_INFO("libsha384test", 0, 1, 1);
...
int main(void){
	...
	//holders
	unsigned char a_hash[SHA384_HASH_LEN];
	char a_buffer[3];
	
	//compute all segments in one pass
	sceSha384Digest((const unsigned char*)string, strlen((const char*)string), a_hash);
	
	//convert to readable and print the resulting digest
	for(i=0;i<SHA384_HASH_LEN;i++){
	...
	//holders
	unsigned char b_hash[SHA384_HASH_LEN];
	char b_buffer[3];
	
	//declare context for sha384 data
	sceSha384Context pContext;
	
	//init sha384 context
	sceSha384BlockInit(&pContext);
	
	//loop through the block of data from start to end, 
	//in this case its a string so we can use strlen to find the end.
	for(i=0;i<(strlen(string));i++){
		//compute segment
		sceSha384BlockUpdate(&pContext, (const unsigned char*)string+i, sizeof(unsigned char));
	}
	
	//get the final digest
	sceSha384BlockResult(&pContext, b_hash);
