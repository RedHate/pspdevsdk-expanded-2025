...
int main(void){
	...
	//holders
	unsigned char a_hash[SHA1_HASH_LEN];
	char a_buffer[3];
	
	//compute all segments in one pass
	sceSha1Digest((const unsigned char*)string, strlen((const char*)string), a_hash);
	...
	sceSha1Context pContext;
	
	//init sha1 context
	sceSha1BlockInit(&pContext);
	
	//loop through the block of data from start to end, 
	//in this case its a string so we can use strlen to find the end.
	for(i=0;i<(strlen(string));i++){
		//compute segment
		sceSha1BlockUpdate(&pContext, (const unsigned char*)string+i, sizeof(unsigned char));
	}
	
	//get the final digest
	sceSha1BlockResult(&pContext, b_hash);
