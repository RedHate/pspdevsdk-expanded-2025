 ...
 * Ultros - 2024
 * 
 * To use libsha256 you must first acquire the libsha256.prx from somewhere, i got mine from "talkman"
...
PSP_MODULE_INFO("libsha256test", 0, 1, 1);
...
int main(void){
	...
	//holders
	unsigned char a_hash[SHA256_HASH_LEN];
	char a_buffer[3];
	
	//compute all segments in one pass
	sceSha256Digest((const unsigned char*)string, strlen((const char*)string), a_hash);
	...
	//holders
	unsigned char b_hash[SHA256_HASH_LEN];
	char b_buffer[3];
	
	//declare context for sha256 data
	sceSha256Context pContext;
	
	//init sha256 context
	sceSha256BlockInit(&pContext);
	
	//loop through the block of data from start to end, 
	//in this case its a string so we can use strlen to find the end.
	for(i=0;i<(strlen(string));i++){
		//compute segment
		sceSha256BlockUpdate(&pContext, (const unsigned char*)string+i, sizeof(unsigned char));
	}
	
	//get the final digest
	sceSha256BlockResult(&pContext, b_hash);
