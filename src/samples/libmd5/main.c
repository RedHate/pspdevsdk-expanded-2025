 ...
 * Ultros - 2024
 * 
 * To use libmd5 you must first acquire the libmd5.prx from somewhere, i got mine from "talkman"
...
PSP_MODULE_INFO("libmd5test", 0, 1, 1);
...
int main(void){
	...
	//holders
	unsigned char a_hash[MD5_HASH_LEN];
	char a_buffer[3];
	
	//compute all segments in one pass
	sceMd5Digest((const unsigned char*)string, strlen((const char*)string), a_hash);
	...
	//holders
	unsigned char b_hash[MD5_HASH_LEN];
	char b_buffer[3];
	
	//declare context for md5 data
	sceMd5Context pContext;
	
	//init md5 context
	sceMd5BlockInit(&pContext);
	
	//loop through the block of data from start to end, 
	//in this case its a string so we can use strlen to find the end.
	for(i=0;i<(strlen(string));i++){
		//compute segment
		sceMd5BlockUpdate(&pContext, (const unsigned char*)string+i, sizeof(unsigned char));
	}
	
	//get the final digest
	sceMd5BlockResult(&pContext, b_hash);
